...
import { isMgdbVideoConfig, mgdbDurationOptions, mgdbPixelLabel, mgdbRatioOptions, normalizeMgdbDuration, normalizeMgdbRatio } from "@/lib/mgdb-video";
import {
    boolConfig,
    isSeedanceFastModel,
...
export function VideoSettingsPanel({ config, onConfigChange, theme, showTitle = true, className = "w-[320px] space-y-4 rounded-2xl px-1 py-0.5" }: VideoSettingsPanelProps) {
    if (isMgdbVideoConfig(config)) {
        return <MgdbVideoSettingsPanel config={config} onConfigChange={onConfigChange} theme={theme} showTitle={showTitle} className={className} />;
    }
...
function MgdbVideoSettingsPanel({ config, onConfigChange, theme, showTitle, className }: VideoSettingsPanelProps) {
    const ratio = normalizeMgdbRatio(config.size);
    const duration = normalizeMgdbDuration(config.videoSeconds);
                    ...
                    <div className="grid grid-cols-3 gap-2.5">
                        {mgdbRatioOptions.map((item) => (
                            ...
                            >
                                ...
                                <span>{item.label}</span>
                                <span className="text-[10px] leading-none opacity-55">{mgdbPixelLabel(item.value)}</span>
                            </button>
                        ))}
                    </div>
                    <div className="text-[11px] leading-4 opacity-55">带参考图时，产物比例以参考图为准；需要精确比例请先裁剪参考图。</div>
                </SettingGroup>
                <SettingGroup title="时长" color={theme.node.muted}>
                    <div className="grid grid-cols-3 gap-2.5">
                        {mgdbDurationOptions.map((value) => (
                            <OptionPill key={value} selected={duration === value} theme={theme} onClick={() => onConfigChange("videoSeconds", String(value))}>
                                {value}s
                            </OptionPill>
                        ))}
                    </div>
                    <div className="text-[11px] leading-4 opacity-55">MGDB 通道按次计费，仅支持参考图与 5/10/15 秒时长。</div>
                </SettingGroup>
            </div>
        </ImageSettingsTheme>
    );
