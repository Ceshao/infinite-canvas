...
import { isMgdbVideoConfig, normalizeMgdbDuration, normalizeMgdbRatio } from "@/lib/mgdb-video";
...
function buildVideoConfig(config: AiConfig, model: string): AiConfig {
    const mgdb = isMgdbVideoConfig({ ...config, model });
    const seedance = !mgdb && isSeedanceVideoConfig({ ...config, model });
    return {
        ...config,
        model,
        videoModel: model,
        size: mgdb ? normalizeMgdbRatio(config.size) : seedance ? normalizeSeedanceRatio(config.size) : normalizeVideoSize(config.size),
        videoSeconds: mgdb ? String(normalizeMgdbDuration(config.videoSeconds)) : normalizeVideoSeconds(config.videoSeconds),
        vquality: normalizeResolution(config.vquality),
